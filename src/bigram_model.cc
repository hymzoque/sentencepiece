...
bool Model::VerifyOutputsEquivalent(absl::string_view expected,
  ...
  auto compute_bigram_model_score =
  ...
  const auto expected_score =
      compute_bigram_model_score(absl::StrSplit(expected, ' '));
  const auto actual_score =
      compute_bigram_model_score(absl::StrSplit(actual, ' '));
...
EncodeResult Model::EncodeOptimized(absl::string_view normalized) const {
  // An optimized Viterbi algorithm for bigram language models. Benchmarking
  ...
  //
  // 1. Because it uses the *bigram* model:
  ...
  //
  // 2. Again, because it uses the *bigram* model, we don’t need to actually
